<template>
    <div class="external-ids">
        ...
        <app-link v-if="show.hasXemNumbering || (show.xemNumbering && show.xemNumbering.length > 0)" :href="`http://thexem.info/search?q=${show.title}`" :title="`http://thexem.info/search?q=${show.title}`">
